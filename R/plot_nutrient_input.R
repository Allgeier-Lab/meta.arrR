...
#'
#' @export
plot_nutrient_input <- function(x, base_size = 10, limits = NULL, ...) {
  ...
  if (inherits(x = x, what = "data.frame")) {
    ...
    input_df <- do.call(rbind, lapply(seq_along(x), function(i) data.frame(meta = i, timestep = 1:length(x[[i]]), input = x[[i]])))
