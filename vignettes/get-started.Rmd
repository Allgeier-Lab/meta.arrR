...
days <- 50 # which((max_i %% ((24 / (min_per_i / 60)) * 1:365)) == 0)
...
# parameters$move_residence_var <- 0.5
...
seafloor_xy <- cbind(id = 1:n, expand.grid(seq(from = -0.75, to = 0.75, length.out = 3),
                                           seq(from = -0.75, to = 0.75, length.out = 3)))

# seafloor_xy <- cbind(id = 1:n, x = c(runif(n = 8, min = -1, max = 0), 
...
plot(metasyst, lambda = parameters$move_lambda)

nutrients_input <- sim_nutr_input(n = n, max_i = max_i, seagrass_each = seagrass_each, 
